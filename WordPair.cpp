/*
 * WordPair.cpp
 * 
 * Description: Models an English word and its translation in another language.
 * 
 * Author: AL
 * Date of last modification: Feb. 2022
 */
 
#include "WordPair.h"

// Constructors
WordPair::WordPair() {
	slang = "";
	translation = "";
}

WordPair::WordPair(string slang) {
	this->slang = slang;
	this->translation = "";
}

WordPair::WordPair(string slang, string translation) {
	this->slang = slang;
	this->translation = translation;
}

// Getters
string WordPair::getEnglish() const {
	return this->slang;
}

string WordPair::getTranslation() const {
	return this->translation;
}

// Setters
void WordPair::setEnglish(string slang) {
	this->slang = slang;
}

void WordPair::setTranslation(string translation) {
	this->translation = translation;
}

// Overloaded Operators
bool WordPair::operator==(const WordPair& rhs) const {

	return (this->slang.compare(rhs.getEnglish()) ) == 0;

} // end of operator==
/*bool WordPair::operator==(const string a) const {

	return true;

} // end of operator==
*/
bool WordPair::operator<(const WordPair& rhs) const {

	return (this->slang.compare(rhs.getEnglish()) ) < 0;
} // end of operator<

bool WordPair::operator>(const WordPair& rhs) const {

	return (this->slang.compare(rhs.getEnglish()) ) > 0;
} // end of operator>

// For testing purposes!
// Description: Prints the content of "wP".
ostream & operator<<(ostream & os, const WordPair & wP) {
	
	os << wP.slang << ":" << wP.translation << endl; 
	     
	return os;

} // end of operator<<